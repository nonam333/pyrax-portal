...
export default function ArticleCard({ article, variant = 'default' }: ArticleCardProps) {
  ...
  return (
    <Link href={`/article/${article.id}`}>
      <article 
        className={`group hover-elevate rounded-lg overflow-hidden bg-card border border-card-border transition-all duration-200 cursor-pointer ${
          isLarge ? 'md:flex' : ''
        }`}
        ...
        <div className={`p-4 ${isLarge ? 'md:w-1/2 md:p-6' : 'space-y-3'}`}>
          <h3 
            className={`font-bold text-card-foreground group-hover:text-primary transition-colors line-clamp-2 ${
              isLarge ? 'text-xl md:text-2xl mb-3' : isCompact ? 'text-sm' : 'text-lg'
            }`}
          ...
          <div className={`flex items-center text-muted-foreground ${
            isCompact ? 'text-xs space-x-3' : 'text-sm space-x-4'
          }`}>
